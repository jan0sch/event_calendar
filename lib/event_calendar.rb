  ...
  module ClassMethods
    ...
    def events_for_date_range(start_d, end_d, find_options = {})
      self.scoped(find_options).find(
        :all,
        :conditions => [ "((? <= #{self.quoted_table_name}.#{self.end_at_field}) OR (#{self.quoted_table_name}.#{self.end_at_field} IS NULL)) AND (#{self.quoted_table_name}.#{self.start_at_field}< ?)", start_d.to_time.utc, end_d.to_time.utc ],
